...
class ProfileController extends Controller
{
    ...
    public function favorateArtists()
    {
        ...
        foreach ($artists as $art) {
            ...
            $promises[] = [
                $this->client->getAsync('?method=artist.getInfo&api_key=' . config('lastfm.last_api_key') . '&mbid=' . $art . '&format=json&limit=10'),
            ];
        }
    ...
    public function favorateAlbums()
    {
        ...
        foreach ($albums as $album) {
            ...
            $promises[] = [
                $this->client->getAsync('?method=album.getInfo&api_key=' . config('lastfm.last_api_key') . '&artist=' . $album->artist_name . '&album=' .  $album->album_name . '&format=json&limit=10'),
            ];
        }
