...
class ArtistController extends Controller
{
    ...
    public function index()
    {
        ...
        $promises = [
            $this->client->getAsync('?method=chart.gettopartists&api_key=' . config('lastfm.last_api_key') . '&format=json&limit=100'),
            $this->client->getAsync('?method=chart.gettoptracks&api_key=' . config('lastfm.last_api_key') . '&format=json&limit=100'),
        ];
    ...
    public function show($mbid)
    {
        ...
        $promise = [
            $this->client->getAsync('?method=artist.getInfo&api_key=' . config('lastfm.last_api_key') . '&mbid=' . $mbid . '&format=json&limit=10'),
        ];
    ...
    public function search($artist)
    {
        ...
        $promise = [
            $this->client->getAsync('?method=artist.search&api_key=' . config('lastfm.last_api_key') . '&artist=' . $artist . '&format=json&limit=10'),
        ];
    ...
    public function destroy($mbid)
    {
        ...
        if ($delete) {
            return response()->json([
                'message' => 'Artist removed from favorite'
            ], 201);
        }
        // If the deletion failed, the function returns a JSON response with an error message and a status code of 500 (Internal Server Error).
            ...
            return response()->json([
                'message' => 'Artist can not be found'
            ], 500);
        }
    }
}
