...
class AlbumController extends Controller
{
        ...
        $validator = $request->validated(); {
            if (!$validator) {
                ...
                return response()->json([
                    'message' => 'Album added to favorite'
                ], 201);
            }
        }
    }
    ...
    {
        $promises = [
            $this->client->getAsync('?method=album.getInfo&api_key=' . config('lastfm.last_api_key') . '&artist=' . $artist . '&album=' . $name . '&format=json'),
    ...
    {
        $promises = [
            $this->client->getAsync('?method=album.search&api_key=' . config('lastfm.last_api_key') . '&album=' . $name . '&format=json'),
        ];
    ...
    {
        $promises = [
            $this->client->getAsync('?method=user.getTopAlbums&api_key=' . config('lastfm.last_api_key') . '&user=' . config('lastfm.last_fm_user') . '&format=json'),
        ];
    ...
    public function destroy($name, $artist)
    {
        ...
        if ($delete) {
            return response()->json([
                'message' => 'Album removed from favorite'
            ], 201);
        } else {
            return response()->json([
                'message' => 'Album can not be found'
            ], 500);
        }
    }
}
